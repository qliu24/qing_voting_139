...
def vgg_arg_scope(padding='SAME', bn=False, weight_decay=0.00005, is_training=False):
...
def vgg_16_part(inputs, is_training=True, spatial_squeeze=True):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
        ...
        # net = slim.conv2d(net, 823, [1, 1], scope='fc8', activation_fn=None)
...
def vgg_16(inputs, spatial_reduce=True):
    """Oxford Net VGG 16-Layers version D Example.
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
        ...
        net = slim.conv2d(net, 4096, [7, 7], padding='VALID', scope='fc6')
        net = slim.conv2d(net, 4096, [1, 1], scope='fc7')
        net = slim.conv2d(net, 1000, [1, 1], scope='fc8', activation_fn=None, normalizer_fn=None)
...
def alexnet(inputs, is_training=True, spatial_squeeze=True):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
        ...
        net = slim.conv2d(net, 50, [1, 1], scope='fc8', activation_fn=None, normalizer_fn=None)
