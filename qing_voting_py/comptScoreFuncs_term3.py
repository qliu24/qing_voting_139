...
def get_score(lfd, lfb, heat_map, vc_part_p, vc_templates):
    ...
    for vc_i in range(max_2):
        for hmnn in range(len(heat_map[vc_i])):
            ...
            for pp in zip(ri, ci):
                ...
                score1 = np.log(hm_ll[vc_i][hmnn][pp[0],pp[1]]) + np.log(vc_part_p[vc_i][hmnn]) - hm_BG_val
                score3 = (np.sum((pp_dist - np.ones((7,7))*1.3)**2)-np.sum((pp_dist - vc_tplt)**2))/5
                det.append(score1+score3)
...
def get_scoreM(lfd, lfb, heat_map_M, vc_part_p, vc_templates):
    rst = []
    for hm, vcP, vcT in zip(heat_map_M, vc_part_p, vc_templates):
        rst.append(get_score(lfd, lfb, hm, vcP, vcT))
