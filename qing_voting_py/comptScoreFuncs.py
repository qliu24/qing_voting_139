...
def dist_to_encode(dist, stride=2, magic_thh=0.046):
    ...
    return (tmp<magic_thh).astype(int)


def comptScores(dist, obj_weight, logZ):
...
def comptScores2(inp, obj_weight, logZ):
    hi,wi,ci = inp.shape
    ho,wo,co = obj_weight.shape
    assert(ci == co)
    if hi > ho:
...
def comptScoresM(dist, obj_weights, logZs, log_priors):
    ...
    score_i = np.zeros(K)
    for kk in range(K):
        logllk = comptScores2(inp, obj_weights[kk], logZs[kk])
