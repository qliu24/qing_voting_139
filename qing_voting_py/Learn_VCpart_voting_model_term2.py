    ...
    for lfd_i,lfd in enumerate(layer_f_dist):
        ...
        for vc_i in range(max_2):
            ...
            if vc_part_cnt[vc_i] == 1:
                ...
                layer_f_pos[lfd_i][vc_i][0] = pos_lsi[np.argmax(det)]
...
K=4
save_name = file_path2+'VCpart_model_{0}_K{1}_term2_blur_coef.pickle'.format(oo, K)
...
for kk in range(K):
    ...
    for itt in range(15):
        ...
        sc, sc_d = get_score(layer_feature_dist_kk, layer_fired_pos, heat_map, vc_part_cnt, vc_part_p)
        print(sc, sc_d)

        layer_fired_pos = get_fired_pos(layer_feature_dist_kk, layer_feature_b, heat_map, vc_part_cnt, vc_part_p)
...
with open(save_name, 'wb') as fh:
    pickle.dump([hm_ls, vcpartp_ls], fh)
