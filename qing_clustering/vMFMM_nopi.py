...
class vMFMM_nopi:
    ...
    def fit(self, features, kappa, max_it, normalized = False, verbose=True):
        ...
        if self.init_method =='random':
            ...
            for i in range(self.cls_num-1):
                ...
                subset_idx = np.random.choice(self.n, siez=(100,), replace=False)
    ...
    def e_step(self):
        # update p
        logP = np.dot(self.features, self.mu.T)*self.kappa2  # n by k
    ...
    def m_step(self):
        ...
        r = np.mean(np.sqrt(np.sum(self.mu**2, axis=1))/self.cls_num)
